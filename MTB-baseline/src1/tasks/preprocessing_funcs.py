...
def preprocess_semeval2010_8(args):
    ...
    data_path = './data/' + args.task + '/dev.txt'
    logger.info("Reading dev file %s..." % data_path)
    with open(data_path, 'r', encoding='utf8') as f:
        text = f.readlines()
    ...
    save_as_pickle('./data/{}/df_dev.pkl'.format(args.task), df_dev)
    logger.info("Finished and saved!")
    
    return df_train, df_test, df_dev, rm
...
class semeval_dataset(Dataset):
    def __init__(self, df, tokenizer, e1_id, e2_id, type, task, model):
        if type != 'train' and type != 'test' and type != 'dev':
            raise ValueError("'type' should be 'train' or 'test' or 'dev'")
...
def load_dataloaders(args):
    ...
    if args.task == 'SemEval' or args.task == 'TACRED' or args.task == 'KBP37' or args.task == 'FewRel':
        ...
        dev_path='./data/{}/df_dev.pkl'.format(args.task)
        test_path = './data/{}/df_test.pkl'.format(args.task)
        if os.path.isfile(relations_path) and os.path.isfile(train_path) and os.path.isfile(test_path) and os.path.isfile(dev_path):
            ...
            df_dev = load_pickle('./data/{}/df_test.pkl'.format(args.task))
            logger.info("Loaded preproccessed data.")
        else:
            df_train, df_test, df_dev, rm = preprocess_semeval2010_8(args)
        ...
        dev_set = semeval_dataset(df_dev, tokenizer=tokenizer, e1_id=e1_id, e2_id=e2_id, type='dev', task=args.task,
                                   model=args.model_no)
        train_length = len(train_set); test_length = len(test_set); dev_length = len(dev_set)
    ...
    return train_loader, test_loader, dev_loader, train_length, test_length, dev_length