...
def train_and_fit(args):
    ...
    for epoch in range(start_epoch, args.num_epochs):
        ...
        for i, data in enumerate(train_loader, 0):
            x, e1_e2_start, labels,_,_,_ = data#labels (batch_size,(NK+1)) NK+1=6
            M = x.shape[1]
            batch_size = x.shape[0]
            x = x.reshape(x.shape[0]*x.shape[1], x.shape[2])
            e1_e2_start = e1_e2_start.reshape(e1_e2_start.shape[0]*e1_e2_start.shape[1], e1_e2_start.shape[2])

            attention_mask = (x != pad_id).float()#(batch_size*6,句子长度)
            token_type_ids = torch.zeros((x.shape[0], x.shape[1])).long()#(batch_size*6,句子长度)
            ...
            outputs = logits.reshape(batch_size, M, logits.shape[1])#(batch_size,6,1536)
            ...
            target_matrix_product = torch.matmul(target_output, anchor_output.permute(0, 2, 1))#(batch_size,1,5) 相似度,test句的预测
            #predic_labels = target_matrix_product[0]/torch.sum(target_matrix_product[0])#(1,5)一个batch的第一句
            predic_labels = target_matrix_product[0]
            true_labels = [labels[0][M-1].item()] #(1) 先转为list再转为tensor
            for j, x in enumerate(target_matrix_product):
                ...
                if (j!=0):
                    ...
                    true_labels.append(labels[j][M-1].item())
            true_labels=torch.LongTensor(true_labels)
            if cuda:
                true_labels=true_labels.cuda()

            loss = criterion(predic_labels, true_labels) #predic_labels(batch_size,5) true_labels(batch_size)
            ...
            total_acc += evaluate_(similar_scores , true_labels, ignore_idx=-1)
    ...
    path = r'/home/fuys/mtb/fr2/result/result.txt'
    doc = open(path, 'a')
