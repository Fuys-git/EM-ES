...
def evaluate_(similar_scores , true_labels, ignore_idx):
    ### ignore index 0 (padding) when calculating accuracy
    #idxs = (labels != ignore_idx).squeeze()
    hits=0
    for k, predic in enumerate(similar_scores):
        if (similar_scores[k] == true_labels[k]):
            hits += 1
    accuracy = hits / (k + 1)
    return accuracy
...
def evaluate_results(net, test_loader, pad_id, cuda):
    ...
    with torch.no_grad():
        for i, data in tqdm(enumerate(test_loader), total=len(test_loader)):
            x, e1_e2_start, labels,_,_,_ = data #labels(batch_size,6)
            M = x.shape[1]  ## M=6
            ...
            acc += evaluate_(similar_scores , true_labels, ignore_idx=-1)
        Acc = acc / (i + 1)
    results = {
        "accuracy": Acc,
    }
