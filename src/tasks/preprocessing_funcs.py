...
import itertools
import numpy as np
from ..transformers.models.bert import BertTokenizer, BertModel, BertForMaskedLM, BertForSequenceClassification
...
class Pad_Sequence():
    ...
    def __call__(self, batch):
        sorted_batch = sorted(batch, key=lambda x: x[0].shape[0], reverse=True) #降序排序
...
class Pad_Sequence2():
    ...
    def __call__(self, batch):
        sorted_batch = sorted(batch, key=lambda x: x[0].size()[1], reverse=True)  # 降序排序
        seqs = [x[0].permute(1,0) for x in sorted_batch]
        seqs_padd = pad_sequence(seqs, batch_first=True, padding_value=self.seq_pad_value)
        seqs_padded = seqs_padd.permute(0,2,1)
        x_lengths = torch.LongTensor([len(x) for x in seqs])
...
def preprocess_fewrel(args, do_lower_case=True):
    ...
    save_as_pickle('./data/relations.pkl', rm)
    df_train['labels'] = df_train.progress_apply(lambda x: rm.rel2idx[x['labels']], axis=1)
   # df_test['labels'] = df_test.progress_apply(lambda x: rm.rel2idx[x['labels']], axis=1)
...
class fewrel_dataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        meta_labels = meta_train_labels + [torch.LongTensor(meta_test_labels)]
        meta_input_padded = pad_sequence(meta_input, batch_first=True, padding_value=self.seq_pad_value).squeeze()
       # x = meta_input_padded.size()[0]*meta_input_padded.size()[1]
       # input=meta_input_padded.reshape( x )

        #return meta_input_padded, e1_e2_start, torch.LongTensor(meta_labels).squeeze()
        return torch.LongTensor(meta_input_padded),\
                e1_e2_start, \
...
def load_dataloaders(args):
    ...
    if args.task == 'semeval':
        ...
        if os.path.isfile(relations_path) and os.path.isfile(train_path) and os.path.isfile(test_path):
            rm = load_pickle('./data/relations.pkl')
            df_train = load_pickle('./data/df_train.pkl')
            df_test = load_pickle('./data/df_test.pkl')
        ...
        PS = Pad_Sequence2(seq_pad_value=tokenizer.pad_token_id, \
                          label_pad_value=tokenizer.pad_token_id, \
                          label2_pad_value=-1)
        train_set = fewrel_dataset(df_train, tokenizer=tokenizer, seq_pad_value=tokenizer.pad_token_id,
                                      e1_id=e1_id, e2_id=e2_id)
        train_loader = DataLoader(train_set, batch_size=args.batch_size, shuffle=True, \
                                  num_workers=0, collate_fn=PS, drop_last=False, pin_memory=False)
        train_length = len(train_loader)
        test_set = fewrel_dataset(df_test, tokenizer=tokenizer, seq_pad_value=tokenizer.pad_token_id,
                                      e1_id=e1_id, e2_id=e2_id)
        test_loader = DataLoader(test_set, batch_size=args.batch_size, shuffle=True, \
                                 num_workers=0, collate_fn=PS, drop_last=False, pin_memory=False)
